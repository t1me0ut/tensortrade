...
class ActionSchemeSimple(ActionScheme):
    ...
    def __init__(self,
                 stop_loss_percentages: Union[List[float], float] = [0.02],
                 take_profit_percentages: Union[List[float], float] = [0.02],
    ...
    def get_order(self, action: int, portfolio: 'Portfolio') -> Optional[Order]:
        ...
        exchange_pair = self.exchange_pairs[0]

        instrument = side.instrument(exchange_pair.pair)
        wallet = portfolio.get_wallet(exchange_pair.exchange.id, instrument=instrument)
        quantity = (wallet.balance.as_float() * instrument).quantize()

        if wallet.balance.as_float() < 10 ** -exchange_pair.pair.base.precision:
            return None
